...
count_24_normalized <- read.csv("denoised/count_24_marker_original.csv",
...
vimp_mat <- matrix(0, nrow=7, ncol=ncol(count_24_normalized_imputed))
rownames(aucs_df) <- rownames(vimp_mat) <- c("Original", "Denoise_4", "Denoise_5",
                         "Denoise_6", "Denoise_7", "Denoise_8", "Denoise_9")
colnames(vimp_mat) <- colnames(count_24_normalized_imputed)

# lasso on original data
rf_result <- fit_rf(y=target, X=count_24_normalized_imputed,
                              seed=myseed)
...
for (j in 1:6){
  ...
  rf_result_denoised <- fit_rf(y=target, X=count_24_normalized_denoised,
                                         seed=myseed)
