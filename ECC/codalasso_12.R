...
count_12_normalized <- read.csv("ECC/denoised/count_12_marker_original.csv",
...
coefs_mat <- matrix(0, nrow=7, ncol=ncol(count_12_normalized_imputed))
rownames(aucs_df) <- rownames(coefs_mat) <- c("Original", "Denoise_4", "Denoise_5",
                         "Denoise_6", "Denoise_7", "Denoise_8", "Denoise_9")
colnames(coefs_mat) <- colnames(count_12_normalized_imputed)

# lasso on original data
lasso_result <- fit_codalasso(y=target_binary, X=count_12_normalized_imputed,
                              lambdas=0.4,
                              seed=myseed)
...
for (j in 1:6){
  ...
  lasso_result_denoised <- fit_codalasso(y=target_binary, X=count_12_normalized_denoised,
