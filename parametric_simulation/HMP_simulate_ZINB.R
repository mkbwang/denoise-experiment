...
# just pick the first three phenotypes
zinb_parameters <- zinb_parameters[1:3]
...
for (j in 1:length(zinb_parameters)){
  ...
  selected_parameter <- zinb_parameters[[j]][1:300, ]
...
colnames(count_mat) <- colnames(mean_mat) <- sprintf("Sample%d", seq(1, nsample*length(count_mat_list)))


output_count_file <- sprintf("parametric_simulation/data/zinb/sim_count_zinb_n%d_d%d_%d.csv", 
                             nsample, dispersion, myseed)
output_mean_file <- sprintf("parametric_simulation/data/zinb/mean_zinb_n%d.csv", 
                            nsample)

if (!file.exists(file.path(folder, output_count_file))){
  write.csv(count_mat, file.path(folder, output_count_file), 
            quote=FALSE)
}

if (!file.exists(file.path(folder, output_mean_file))){
